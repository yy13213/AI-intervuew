...
"""
简历深挖功能模块
从test_result_questions.json读取简历深挖题目，使用QA-API进行面试问答
"""
...
class ResumeDigging:
    ...
    def get_resume_questions(self):
        """获取简历深挖题目"""
        if not self.questions_data:
            print("❌ 请先加载题目文件")
            return []
        
        resume_questions = self.questions_data.get('questions', {}).get('简历深挖', [])
        
        if not resume_questions:
            print("❌ 未找到简历深挖题目")
            return []
        
        print(f"📝 找到 {len(resume_questions)} 个简历深挖题目:")
    ...
    def conduct_resume_interview(self, num_questions=2):
        ...
        # 获取简历深挖题目
        resume_questions = self.get_resume_questions()
        ...
        # 限制题目数量
        questions_to_ask = resume_questions[:num_questions]
        print(f"\n🎯 将进行 {len(questions_to_ask)} 个简历深挖问题的面试")
    ...
    def show_all_questions(self):
        ...
        for i, q in enumerate(resume_questions):
            ...
            print(f"关注领域: {q.get('focus_area', '通用')}")

def main():
    """主函数"""
    resume_digging = ResumeDigging()
    
    print("=== 简历深挖功能 ===")
    
    # 1. 加载题目
    if not resume_digging.load_questions():
        return
    
    # 2. 显示题目概览
    resume_digging.show_all_questions()
    
    # 3. 进行面试（前两题）
    print(f"\n🚀 开始简历深挖面试...")
    resume_digging.conduct_resume_interview(num_questions=2)
